...
USE THE BUDGET — BREADTH FIRST, THEN DENSITY (do NOT spend signs lengthening one trail): the agent told you how many signs they have; get close to that number, but spend it in this strict priority order:
  1. COVER THE DIRECTIONS TRAFFIC ACTUALLY COMES FROM. Buyers arrive from more than one direction — the main arterial from BOTH ends (e.g. northbound AND southbound), plus any other genuine arterial or cross-street that feeds the area. Open each real approach at its followable minimum (entry + its turns + property) BEFORE deepening any single one. More signs must mean MORE directions covered — never one trail running farther out.
  2. SATURATE THE NEAR-HOUSE BLOCK. A sign or two within the final ~1/8 mile, plus a sign at each end of the property's own street — these serve every approach at once and are the highest value per sign.
  3. ONLY THEN add confirmation signs on long gaps within the funded approaches, so no trail goes quiet for more than a few blocks.
Never run a single approach out to the edge of the map to use up signs — an approach anchored more than ~0.5 mile from the property is usually too far; cover another direction instead. Don't pad with redundant signs, but don't leave the budget unused while a real direction or near-house spot is still uncovered. A driver should never be guessing.
